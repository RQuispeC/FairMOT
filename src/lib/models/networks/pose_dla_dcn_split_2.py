...
class DLASeg(nn.Module):
    ...
    def forward(self, x):
        ...
        shared_repr = self.attent_shared(y[-1])
        dete_repr = self.attent_detect(y[-1])
        reid_repr = self.attent_reid(y[-1])
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
  "base model using attention layers to split f into shared, deid and detection heads"
