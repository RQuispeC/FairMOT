...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads, pretrained, down_ratio, final_kernel,
        ...
        self.detection_heads = ['hm','wh', 'reg']
        ...
        self.detect_bn = nn.Sequential(
                  ...
                  nn.BatchNorm2d(channels[self.first_level], momentum=BN_MOMENTUM),
                  nn.ReLU(inplace=True),
                  nn.Conv2d(channels[self.first_level], channels[self.first_level],
                    kernel_size=3, padding=1, bias=True),
                  nn.BatchNorm2d(channels[self.first_level], momentum=BN_MOMENTUM),
    ...
    def forward(self, x):
        ...
        dete_repr_mot = self.detect_bn(dete_repr_mot)
        reid_repr_mot = self.reid_bn(reid_repr_mot)
        z = {}
        for head in self.heads:
...
def get_pose_net(num_layers, heads, head_conv=256, down_ratio=4):
  ...
  + (conv, relu)**2 for reid and detection branches after joining with shared space(for ablation analysis of split_8 and effects of bn)
  """
